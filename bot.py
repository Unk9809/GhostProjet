...
def main():
    ...
    bot_token = '5280147082:AAH8iRFUKENnQcTDNzggY_l0dYD2PxKHFYQ'
    tl_admin_user = 'Uk98090000'
    # end
    ...
    async def click_handler(ev: events.NewMessage.Event): await dispatch_event(ev)
    @bot.on(events.CallbackQuery())
    async def click_handler(ev: events.NewMessage.Event): await dispatch_event(ev)
    @bot.on(events.NewMessage())
    async def process(ev: events.NewMessage.Event): await dispatch_event(ev)
